...
class SettingsView(BaseView):
    ...
    def update_kwargs(self):
        # User settings
        self.kwargs["DEFAULT_SETTINGS_PY_PATH"] = self.handle_slash(
            self.DEFAULT_SETTINGS_PY_PATH
        )
        self.kwargs["SCRAPYDWEB_SETTINGS_PY_PATH"] = self.handle_slash(
            self.SCRAPYDWEB_SETTINGS_PY_PATH
        )
        ...
        # ScrapydWeb
        self.kwargs["scrapydweb_server"] = self.json_dumps(
            dict(
        ...
        self.kwargs["enable_https_details"] = self.json_dumps(
            dict(
        ...
        # Scrapy
        self.kwargs["SCRAPY_PROJECTS_DIR"] = (
            self.handle_slash(self.SCRAPY_PROJECTS_DIR) or "''"
        )

        # Scrapyd
        servers = defaultdict(list)
        for group, server, auth in zip(
            self.SCRAPYD_SERVERS_GROUPS,
            self.SCRAPYD_SERVERS,
            self.SCRAPYD_SERVERS_AUTHS,
        ):
            _server = (
                "%s:%s@%s" % (self.protect(auth[0]), self.protect(auth[1]), server)
                if auth
                else server
            )
        ...
        self.kwargs["LOCAL_SCRAPYD_LOGS_DIR"] = (
            self.handle_slash(self.LOCAL_SCRAPYD_LOGS_DIR) or "''"
        )
        ...
        self.kwargs["logparser_settings_py_path"] = self.handle_slash(
        ...
        # Run Spider
        self.kwargs["run_spider_details"] = self.json_dumps(
            dict(
        ...
        # Page Display
        self.kwargs["page_display_details"] = self.json_dumps(
            dict(
        ...
        # Send text
        self.kwargs["slack_details"] = self.json_dumps(
            dict(
                SLACK_TOKEN=self.protect(self.SLACK_TOKEN),
                SLACK_CHANNEL=self.SLACK_CHANNEL,
            )
        )
        self.kwargs["telegram_details"] = self.json_dumps(
            dict(
                TELEGRAM_TOKEN=self.protect(self.TELEGRAM_TOKEN),
                TELEGRAM_CHAT_ID=self.TELEGRAM_CHAT_ID,
            )
        )
        self.kwargs["email_details"] = self.json_dumps(
            dict(
                EMAIL_SUBJECT=self.EMAIL_SUBJECT,
            )
        )
        self.kwargs["email_sender_recipients"] = self.json_dumps(
            dict(
        ...
        self.kwargs["email_smtp_settings"] = self.json_dumps(
            dict(
        ...
        self.kwargs["alert_working_time"] = self.json_dumps(
            [
                dict(
                    ALERT_WORKING_DAYS="%s"
                    % sorted(
                        self.ALERT_WORKING_DAYS
                    ),  # stringify making it displayed in a line
        ...
        self.kwargs["database_details"] = self.json_dumps(
            dict(
                APSCHEDULER_DATABASE_URI=self.hide_account(
                    self.APSCHEDULER_DATABASE_URI
                ),
                SQLALCHEMY_DATABASE_URI=self.hide_account(self.SQLALCHEMY_DATABASE_URI),
                SQLALCHEMY_BINDS_METADATA=self.hide_account(
                    self.SQLALCHEMY_BINDS["metadata"]
                ),
