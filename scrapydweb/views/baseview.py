...
from ..vars import (
    ALLOWED_SCRAPYD_LOG_EXTENSIONS,
    APSCHEDULER_DATABASE_URI,
    DATA_PATH,
    DEMO_PROJECTS_PATH,
    DEPLOY_PATH,
    PARSE_PATH,
...
class BaseView(View):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.DEFAULT_SETTINGS_PY_PATH = app.config.get(
            "DEFAULT_SETTINGS_PY_PATH",
            os.path.join(app.root_path, "default_settings.py"),
        )
        self.SCRAPYDWEB_SETTINGS_PY_PATH = app.config.get(
            "SCRAPYDWEB_SETTINGS_PY_PATH",
            os.path.join(app.root_path, "scrapydweb_settings_v10.py"),
        )
        self.MAIN_PID = app.config.get("MAIN_PID", os.getpid())
        self.LOGPARSER_PID = app.config.get("LOGPARSER_PID")
        self.POLL_PID = app.config.get("POLL_PID")
        ...
        # Scrapyd
        self.SCRAPYD_SERVERS = app.config.get("SCRAPYD_SERVERS", []) or [
            "127.0.0.1:6800"
        ]
        ...
        self.SCRAPYD_SERVERS_AUTHS = app.config.get("SCRAPYD_SERVERS_AUTHS", []) or [
            None
        ]
        self.SCRAPYD_SERVERS_PUBLIC_URLS = (
            app.config.get("SCRAPYD_SERVERS_PUBLIC_URLS", None)
        ...
        self.SCRAPYD_LOG_EXTENSIONS = (
            app.config.get("SCRAPYD_LOG_EXTENSIONS", [])
        ...
        # Run Spider
        self.SCHEDULE_EXPAND_SETTINGS_ARGUMENTS = app.config.get(
            "SCHEDULE_EXPAND_SETTINGS_ARGUMENTS", False
        )
        self.SCHEDULE_CUSTOM_USER_AGENT = app.config.get(
        ...
        self.SCHEDULE_CONCURRENT_REQUESTS = app.config.get(
            "SCHEDULE_CONCURRENT_REQUESTS", None
        )
        self.SCHEDULE_DOWNLOAD_DELAY = app.config.get("SCHEDULE_DOWNLOAD_DELAY", None)
        self.SCHEDULE_ADDITIONAL = app.config.get(
            "SCHEDULE_ADDITIONAL", SCHEDULE_ADDITIONAL
        )
        ...
        self.DAEMONSTATUS_REFRESH_INTERVAL = app.config.get(
        ...
        self.EMAIL_SUBJECT = (
            app.config.get("EMAIL_SUBJECT", "") or "Email from #scrapydweb"
        )
        ...
        for key in self.ALERT_TRIGGER_KEYS:
            setattr(
                self,
                "LOG_%s_THRESHOLD" % key,
                app.config.get("LOG_%s_THRESHOLD" % key, 0),
            )
            setattr(
                self,
                "LOG_%s_TRIGGER_STOP" % key,
                app.config.get("LOG_%s_TRIGGER_STOP" % key, False),
            )
            setattr(
                self,
                "LOG_%s_TRIGGER_FORCESTOP" % key,
                app.config.get("LOG_%s_TRIGGER_FORCESTOP" % key, False),
            )
        ...
        assert (
            0 < self.node <= self.SCRAPYD_SERVERS_AMOUNT
        ), "node index error: %s, which should be between 1 and %s" % (
            self.node,
        ...
        self.FEATURES += (
            "D" if handle_metadata().get("jobs_style") == "database" else "C"
        )
        self.FEATURES += (
            "d" if self.SCRAPY_PROJECTS_DIR != self.DEMO_PROJECTS_PATH else "-"
        )
        ...
        self.any_running_apscheduler_jobs = any(
            job.next_run_time for job in self.scheduler.get_jobs(jobstore="default")
        )
    ...
    @staticmethod
    def json_dumps(
        obj, sort_keys=True, indent=4, ensure_ascii=False, as_response=False
    ):
        # flask.jsonify
        ...
        try:
            ...
            if as_json:
                ...
                try:
                    ...
                    r_json = (
                        r.json()
                    )  # PY3: json.decoder.JSONDecodeError  PY2: exceptions.ValueError
                    ...
                    if r.status_code != 200 or (check_status and status != self.OK):
                        self.logger.error(
                            "!!!!! (%s) %s: %s", r.status_code, status, url
                        )
                    else:
                        self.logger.debug(
                            "<<<<< (%s) %s: %s", r.status_code, status, url
                    ...
                    if dumps_json:
                        self.logger.debug(
                            "Got json from %s: %s", url, self.json_dumps(r_json)
                        )
                    else:
                        self.logger.debug(
                            "Got keys from (%s) %s %s: %s",
                            r_json.get("status_code"),
                            r_json.get("status"),
                            url,
                            r_json.keys(),
                        )
    ...
    def update_g(self):
        ...
        g.url_jobs_list = [
            url_for("jobs", node=node, ui=self.UI)
            for node in range(1, self.SCRAPYD_SERVERS_AMOUNT + 1)
        ]
        g.multinode = (
            '<label title="multinode">'
        ...
        if not self.USE_MOBILEUI:
            ...
            g.scheduler_state_paused = (
                self.scheduler.state == STATE_PAUSED
                and self.any_running_apscheduler_jobs
            )
            g.scheduler_state_running = (
