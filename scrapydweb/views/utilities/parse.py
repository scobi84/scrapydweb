...
def source(filename):
    return send_from_directory(
        PARSE_PATH, filename, mimetype="text/plain", cache_timeout=0
    )
    ...
    def dispatch_request(self, **kwargs):
        if self.POST:
            ...
            if file.filename.rpartition(".")[-1] not in ALLOWED_EXTENSIONS:
                flash(
                    "Only file type of %s is supported" % ALLOWED_EXTENSIONS, self.WARN
                )
                return redirect(request.url)
            ...
            url_parse_demo = url_for(
                ".uploaded", node=self.node, filename="ScrapydWeb_demo.log"
            )
...
class UploadedLogView(BaseView):
    ...
    def dispatch_request(self, **kwargs):
        try:
            ...
            with io.open(
                os.path.join(self.PARSE_PATH, self.filename),
                encoding="utf-8",
                errors="ignore",
            ) as f:
                self.text = f.read()
    ...
    def get_job_info(self):
        ...
        m_spider = re.search(
            r"\[([^.]+?)\]\s+(?:DEBUG|INFO|WARNING|ERROR|CRITICAL)", self.text
        )
        ...
        self.job = (
            m_job.group(1)
            if m_job
            else (self.filename.rpartition(".")[0] or self.filename)
        )
