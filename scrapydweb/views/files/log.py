...
class LogView(BaseView):
    def __init__(self):
        ...
        self.url = "http://{}/logs/{}/{}/{}".format(
            self.SCRAPYD_SERVER, self.project, self.spider, self.job
        )
        self.log_path = os.path.join(
            self.LOCAL_SCRAPYD_LOGS_DIR, self.project, self.spider, self.job
        )
        ...
        self.json_path = os.path.join(
            self.LOCAL_SCRAPYD_LOGS_DIR,
            self.project,
            self.spider,
            job_without_ext + ".json",
        )
        self.json_url = "http://{}/logs/{}/{}/{}.json".format(
            self.SCRAPYD_SERVER, self.project, self.spider, job_without_ext
        )
        ...
        if self.opt == "report":
            ...
            self.template = "scrapydweb/%s%s.html" % (
                self.opt,
                "_mobileui" if self.USE_MOBILEUI else "",
            )
        self.kwargs = dict(
            node=self.node,
    ...
    def dispatch_request(self, **kwargs):
        ...
        if self.report_logparser:
            ...
            if self.status_code < 100 or self.stats:
                ...
                status_code = self.status_code
            return (
                self.json_dumps(self.stats or dict(status="error"), as_response=True),
                status_code,
            )
        else:
            self.update_kwargs()
            if (
                self.ENABLE_MONITOR and self.POST
            ):  # Only poll.py would make POST request
                self.monitor_alert()
    ...
    def read_local_stats_by_logparser(self):
        ...
        try:
            ...
            self.logger.error(
                "Fail to read local stats from %s: %s", self.json_path, err
            )
            return
        else:
            if js.get("logparser_version") != self.LOGPARSER_VERSION:
                msg = "Mismatching logparser_version %s in local stats" % js.get(
                    "logparser_version"
                )
            ...
            msg = "Using local stats: LogParser v%s, last updated at %s, %s" % (
                js["logparser_version"],
                js["last_update_time"],
                self.handle_slash(self.json_path),
            )
    ...
    def request_stats_by_logparser(self):
        ...
        status_code, js = self.make_request(
            self.json_url, auth=self.AUTH, as_json=True, dumps_json=False
        )
        if status_code != 200:
            self.logger.error(
                "Fail to request stats from %s, got status_code: %s",
                self.json_url,
                status_code,
            )
            if self.IS_LOCAL_SCRAPYD_SERVER and self.ENABLE_LOGPARSER:
                flash(
                    "Request to %s got code %s, wait until LogParser parses the log. "
                    % (self.json_url, status_code),
                    self.INFO,
                )
            else:
                flash(
                    (
                        "'pip install logparser' on host '%s' and run command 'logparser'. "
                        "Or wait until LogParser parses the log. "
                    )
                    % self.SCRAPYD_SERVER,
                    self.WARN,
                )
            ...
            msg = (
                "'pip install --upgrade logparser' on host '%s' to update LogParser to v%s"
        ...
        for ext in self.SCRAPYD_LOG_EXTENSIONS:
            ...
            if os.path.exists(log_path):
                if tarfile.is_tarfile(log_path):
                    self.logger.debug(
                        "Ignore local tarfile and use requests instead: %s", log_path
                    )
                    break
    ...
    def request_scrapy_log(self):
        for ext in self.SCRAPYD_LOG_EXTENSIONS:
            url = self.url + ext
            self.status_code, self.text = self.make_request(
                url, auth=self.AUTH, as_json=False
            )
            if self.status_code == 200:
                self.url = url
            ...
            msg = "Fail to request logfile from %s with extensions %s" % (
                self.url,
                self.SCRAPYD_LOG_EXTENSIONS,
            )
    ...
    def simplify_stats_for_report(self):
        ...
        try:
            for key in self.stats["log_categories"]:
                self.stats["log_categories"][key] = dict(
                    count=self.stats["log_categories"][key]["count"]
                )
        except KeyError:
            pass
        try:
            self.stats["latest_matches"] = dict(
                latest_item=self.stats["latest_matches"]["latest_item"]
            )
        except KeyError:
            pass
    ...
    def keep_stats_for_report(self):
        ...
        if self.job_key in od:
            return
        if (
            self.stats.get("shutdown_reason", self.NA) == self.NA
        ...
        if set(self.stats.keys()) == REPORT_KEYS_SET:
            ...
            self.logger.debug(
                "%s keys in job_finished_report_dict[%s]", len(od), self.node
            )
    ...
    def read_stats_for_report(self):
        try:
            ...
            self.logger.debug(
                "%s not found in job_finished_report_dict[%s]", self.job_key, self.node
            )
        else:
            self.logger.debug(
                "%s found in job_finished_report_dict[%s]", self.job_key, self.node
            )
            self.logparser_valid = True
    ...
    def mkdir_spider_path(self):
        node_path = os.path.join(
            self.STATS_PATH,
            re.sub(
                self.LEGAL_NAME_PATTERN, "-", re.sub(r"[.:]", "_", self.SCRAPYD_SERVER)
            ),
        )
    ...
    def backup_stats(self):
        ...
        try:
            with io.open(
                self.backup_stats_path, "w", encoding="utf-8", errors="ignore"
            ) as f:
                f.write(self.json_dumps(self.stats))
        except Exception as err:
            self.logger.error(
                "Fail to backup stats to %s: %s" % (self.backup_stats_path, err)
            )
            try:
    ...
    def load_backup_stats(self):
        ...
        try:
            ...
            self.logger.error(
                "Fail to load backup stats from %s: %s", self.backup_stats_path, err
            )
        else:
            if js.get("logparser_version") != self.LOGPARSER_VERSION:
                msg = "Mismatching logparser_version %s in backup stats" % js.get(
                    "logparser_version"
                )
                self.logger.warning(msg)
                flash(msg, self.WARN)
            ...
            msg = "Using backup stats: LogParser v%s, last updated at %s, %s" % (
                js["logparser_version"],
                js["last_update_time"],
                self.handle_slash(self.backup_stats_path),
            )
            self.logger.info(msg)
            flash(msg, self.WARN)
    ...
    def update_kwargs(self):
        if self.utf8_realtime:
            ...
            if (
                self.kwargs["finish_reason"] == self.NA
        ...
        if self.with_ext and self.job.endswith(".json"):
            ...
            if self.SCRAPYD_SERVER_PUBLIC_URL:
                self.kwargs["url_source"] = re.sub(
                    r"^http.*?/logs/",
                    self.SCRAPYD_SERVER_PUBLIC_URL + "/logs/",
                    self.url,
                )
            else:
                self.kwargs["url_source"] = self.url
            self.kwargs["url_opt_opposite"] = url_for(
                "log",
                node=self.node,
    ...
    def monitor_alert(self):
        ...
        (
            self.job_stats_previous,
            self.triggered_list,
            self.has_been_stopped,
            self.last_send_timestamp,
        ) = job_data
        self.logger.debug(job_data_dict)
        self.job_stats = [
            self.kwargs["log_categories"][k.lower() + "_logs"]["count"]
            for k in self.ALERT_TRIGGER_KEYS
        ]
        self.job_stats.extend(
            [self.kwargs["pages"] or 0, self.kwargs["items"] or 0]
        )  # May be None by LogParser
        self.job_stats_diff = [
            j - i for i, j in zip(self.job_stats_previous, self.job_stats)
        ]
    ...
    def set_email_content_kwargs(self):
        ...
        self.email_content_kwargs["url_stats"] = (
            request.url + "%sui=mobile" % "&" if request.args else "?"
        )

        for idx, key in enumerate(EMAIL_CONTENT_KEYS):
            if self.job_stats_diff[idx]:
                self.email_content_kwargs[key] = "%s + %s" % (
                    self.job_stats_previous[idx],
                    self.job_stats_diff[idx],
                )
            else:
        ...
        for k in ["latest_crawl", "latest_scrape", "latest_log"]:
            ...
            self.email_content_kwargs[k] = (
                self.NA if ts == 0 else "%s secs ago" % int(now_timestamp - ts)
            )

        self.email_content_kwargs["current_time"] = self.get_now_string(True)
        self.email_content_kwargs["logparser_version"] = self.kwargs[
            "logparser_version"
        ]
        self.email_content_kwargs["latest_item"] = (
            self.kwargs["latest_matches"]["latest_item"] or self.NA
        )
    ...
    def set_monitor_flag(self):
        if self.ON_JOB_FINISHED and self.job_finished:
            ...
            for idx, key in enumerate(self.ALERT_TRIGGER_KEYS):
                if (
                    0
                    < getattr(self, "LOG_%s_THRESHOLD" % key, 0)
                    <= self.job_stats[idx]
                    and not self.triggered_list[idx]
                ):
                    self.triggered_list[idx] = True
                    self.email_content_kwargs[
                        "log_%s_count" % key.lower()
                    ] += " triggered!!!"
                    if getattr(self, "LOG_%s_TRIGGER_FORCESTOP" % key):
                        self.flag = (
                            "%s_ForceStop" % key
                            if "_ForceStop" not in self.flag
                            else self.flag
                        )
                        to_forcestop = True
                    elif (
                        getattr(self, "LOG_%s_TRIGGER_STOP" % key)
                        and not self.has_been_stopped
                    ):
                        self.flag = (
                            "%s_Stop" % key if "Stop" not in self.flag else self.flag
                        )
        ...
        if (
            not self.flag
            and 0
            < self.ON_JOB_RUNNING_INTERVAL
            <= time.time() - self.last_send_timestamp
        ):
            self.flag = "Running"
    ...
    def send_alert(self):
        if (
            self.flag
            and date.isoweekday(date.today())
            in self.ALERT_WORKING_DAYS  # date.isoweekday(datetime.now())
            ...
            kwargs = dict(
                ...
                latest_item=self.kwargs["latest_matches"]["latest_item"][:100]
                or self.NA,
            )
            subject = "{flag} [{pages}p, {items}i] {job_key} {latest_item} #scrapydweb".format(
                **kwargs
            )
            self.EMAIL_KWARGS["subject"] = subject
            self.EMAIL_KWARGS["content"] = self.json_dumps(
                self.email_content_kwargs, sort_keys=False
            )
            ...
            if self.ENABLE_SLACK_ALERT:
                ...
                _url = url_for(
                    "sendtextapi", opt="slack", channel_chatid_subject=None, text=None
                )
    ...
    def handle_data(self):
        if self.flag:
            ...
            self.logger.debug(
                "Previous job_data['%s'] %s", self.job_key, job_data_dict[self.job_key]
            )
            job_data_dict[self.job_key] = (
                self.job_stats,
                self.triggered_list,
                self.has_been_stopped,
                time.time(),
            ...
            self.logger.debug(
                "Updated  job_data['%s'] %s", self.job_key, job_data_dict[self.job_key]
            )
